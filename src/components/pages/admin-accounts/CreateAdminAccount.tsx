...
export const CreateAdminAccountModal: React.FC<
...
> = ({ isOpen, onClose }) => {
  ...
  const { handleSubmit, isValid, register, resetForm, setFieldValue, values } =
    useFormikWrapper({
      initialValues: {
        name: "",
        email: "",
        gender: "",
        permission: ["view"] as string[],
      },
      ...
      <div className="fixed inset-0 z-10 w-screen overflow-scroll scrollbar-hide duration-300 ease-out transition-opacity data-[closed]:opacity-0 bg-grey-dark-4/70">
        <div className="flex flex-col min-h-full items-center p-3 justify-end md:justify-center">
          <DialogPanel
            ...
            transition
            className="flex flex-col gap-5 justify-between w-full max-w-[37.75rem] rounded-lg bg-white p-4 md:p-5 backdrop-blur-2xl duration-300 ease-out transform data-[closed]:translate-y-full"
          >
                  ...
                  {permissions.map((item) => {
                    ...
                    return (
                      <Field
                        key={item.value}
                        className={cn(
                          "flex items-center gap-2 border py-2 px-4 rounded transition-all duration-300 ease-out",
                          isChecked
                            ? "border-accent-primary bg-light-red"
                            : "border-blue-4"
                        )}
                      >
                        <Checkbox
                          checked={isChecked}
                          disabled={item.value === "view"}
