...
import { useAddSermonNote } from "@/services/hooks/mutations";
...
export const CreateSermonModal: React.FC<CreateSermonModalProps> = ({ isOpen, onClose }) => {
    const { mutate, isPending } = useAddSermonNote(() => close())
    ...
    const { handleSubmit, isValid, register, resetForm, setFieldValue, values } = useFormikWrapper({
        ...
        onSubmit: () => {
            ...
            mutate({ sermon_date: format(sermon_date, "yyyy-MM-dd"), ...rest })
        }
    })
    const close = () => {
        onClose();
                ...
                <div className="flex flex-col min-h-full items-center p-3 justify-end md:justify-center">
                    <DialogPanel as="form" onSubmit={handleSubmit} transition className="flex flex-col gap-5 justify-between w-full max-w-[37.75rem] rounded-lg bg-white p-4 md:p-5 backdrop-blur-2xl duration-300 ease-out transform data-[closed]:translate-y-full">
                        <div className="flex items-center justify-between gap-3">
                            <DialogTitle className="font-bold text-text-primary text-xl">Create Sermon Note</DialogTitle>
