...
import type { FetchedConnectGroupCountStatusType, FetchedConnectGroupCountType, FetchedConnectGroupType } from "@/types/connect-group"
...
export const ConnectGroupsPage: React.FC = () => {
    ...
    const { data: count, isLoading: isLoadingGroupsCount } = useGetConnectGroups<FetchedConnectGroupCountType>({ q: value, component: "count" })
    const { data: countStatus } = useGetConnectGroups<FetchedConnectGroupCountStatusType>({ component: "count-status" })
    const { data, isLoading } = useGetConnectGroups<FetchedConnectGroupType[]>({ q: value, page: page.toString(), item_per_page: itemsPerPage.toString() })
    ...
    const requestCards = useMemo(() => {
        return [
            { label: "Total Connect Groups", value: countStatus?.total_count || 0, icon: "lucide:life-buoy" },
            { label: "Members", value: countStatus?.total_member || 0, icon: "lucide:users" },
            { label: "Head of Group", value: countStatus?.total_admin || 0, icon: "lucide:users" },
        ]
    }, [countStatus?.total_admin, countStatus?.total_count, countStatus?.total_member])
        ...
        <div className="flex flex-col gap-5 px-4 pt-3 md:pt-5 pb-5 md:pb-10 view-page-container overflow-y-scroll">
            ...
            <div className="flex flex-col md:flex-row gap-y-3 md:items-center justify-between">
                <div className="flex-1 md:max-w-96">
                    <SearchInput onChange={onChangeHandler} placeholder="Search connect group name" />
                </div>
            ...
            <RenderIf condition={!isLoading && !isLoadingGroupsCount}>
                <Table
                    ...
                    onClick={({ original }) => navigate(`/connect-groups/${original?.connectgroup_id}`)}
