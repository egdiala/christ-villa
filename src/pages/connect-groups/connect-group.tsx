...
import {
  ConnectGroupFilter,
  EditConnectGroupModal,
  DeleteConnectGroupModal,
  RemoveMemberModal,
  MakeAdminModal,
} from "@/components/pages/connect-groups";
...
export const ConnectGroupPage: React.FC = () => {
  ...
  const { data, isLoading } =
    useGetConnectGroup<FetchedConnectGroupCountStatusType>({
      id: id as string,
      component: "count-status",
    });
  const { data: members, isLoading: isLoadingMembers } = useGetConnectGroup<
    FetchedConnectGroupMemberType[]
  >({
    id: id as string,
    page: page.toString(),
  ...
  const { data: membersCount, isLoading: isLoadingMembersCount } =
    useGetConnectGroup<{ total: number }>({
      id: id as string,
      component: "count",
    });
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }: { row: any }) => {
        const item = row.original as FetchedConnectGroupMemberType;
        return <div className="capitalize">{item?.account_type}</div>;
      },
    },
    {
      header: () => "Status",
      ...
      cell: ({ row }: { row: any }) => {
        ...
        return (
          <span
            className={cn(
              item?.status === 1 ? "text-green-base" : "text-accent-primary",
              "font-medium text-sm"
            )}
          >
            {item?.status === 1 ? "Approved" : "Suspended"}
          </span>
        );
      },
    },
    ...
    <Fragment>
      <RenderIf condition={!isLoading}>
        <div className="flex flex-col gap-5 px-4 pt-3 md:pt-5 pb-5 md:pb-10 view-page-container overflow-y-scroll">
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <RenderIf condition={permission.includes("delete")}>
                  ...
                  >
                    <Icon
                      icon="lucide:trash"
                      className="group-hover:text-white text-accent-primary size-4"
                    />
                    <span className="group-hover:text-white text-accent-primary">
                      Delete Group
                    </span>
                  </TableAction>
                </RenderIf>
          ...
          <MakeAdminModal
            isOpen={toggleModals.openMakeAdmin}
            member={toggleModals.member!}
            onClose={() =>
              setToggleModals((prev) => ({
                ...prev,
                openMakeAdmin: false,
          ...
          />
          <RemoveMemberModal
            isOpen={toggleModals.openRemoveMember}
            member={toggleModals.member!}
            onClose={() =>
              setToggleModals((prev) => ({
                ...prev,
          ...
          />
          <DeleteConnectGroupModal
            isOpen={toggleModals.openDeleteGroup}
            connectGroup={data!}
            onClose={() =>
              setToggleModals((prev) => ({ ...prev, openDeleteGroup: false }))
            }
          />
          <EditConnectGroupModal
            isOpen={toggleModals.openEditGroup}
            connectGroup={data!}
            onClose={() =>
              setToggleModals((prev) => ({ ...prev, openEditGroup: false }))
            }
          />
        </div>
      </RenderIf>
