...
import { CreateChurchCalendarModal, DeleteChurchCalendarModal } from "@/components/pages/church-calendar";
...
export const ChurchCalendarPage = () => {
    ...
    const { data: calendarsCount, isLoading: isLoadingCount } = useGetCalendars<{ total: number; }>({ component: "count" })
    const { data: calendars, isLoading } = useGetCalendars<FetchedCalendarType[]>({ page: page.toString(), item_per_page: itemsPerPage.toString() })
                            ...
                            <MenuItems transition anchor="bottom end" className="w-52 origin-top-right rounded-xl bg-white px-2.5 py-3.5 transition duration-100 ease-out [--anchor-gap:var(--spacing-1)] focus:outline-none data-[closed]:scale-95 data-[closed]:opacity-0">
                                <MenuSection className="space-y-4">
                                    ...
                                    <div className="grid gap-1">
                                        <MenuItem as="button" type="button" className="flex items-center w-full rounded hover:bg-red-5 px-2 py-1.5 text-sm/6 text-text-secondary" onClick={() => window.open(item?.url, "_blank")}>
                                            View
                                        </MenuItem>
                                        <MenuItem as="button" type="button" className="flex items-center w-full rounded hover:bg-red-5 px-2 py-1.5 text-sm/6 text-text-secondary" onClick={() => setToggleModals((prev) => ({ ...prev, openDeleteCalendar: true, activeCalendar: item }))}>
                                            Delete
                                        </MenuItem>
                                    </div>
                                </MenuSection>
        ...
        <div className="flex flex-col gap-5 px-4 pt-3 md:pt-5 pb-5 md:pb-10 view-page-container overflow-y-scroll">
            <div className="flex flex-col md:flex-row gap-3 md:items-center justify-end">
                <div className="flex items-center gap-4 w-full sm:w-auto">
                    ...
                    <TableAction type="button" theme="primary" onClick={() => setToggleModals((prev) => ({ ...prev, openCreateCalendar: true }))} block>
            ...
            <CreateChurchCalendarModal isOpen={toggleModals?.openCreateCalendar} onClose={() => setToggleModals((prev) => ({ ...prev, openCreateCalendar: false }))} />
            <DeleteChurchCalendarModal isOpen={toggleModals?.openDeleteCalendar} item={toggleModals.activeCalendar!} onClose={() => setToggleModals((prev) => ({ ...prev, openDeleteCalendar: false, activeCalendar: null }))} />
        </div>
    )
}