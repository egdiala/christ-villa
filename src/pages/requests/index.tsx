...
export const RequestsPage: React.FC = () => {
  ...
  const columns = [
    {
      ...
      cell: ({ row }: { row: any }) => {
        ...
        return <div className="capitalize">{item?.user_data?.name || "-"}</div>;
      },
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }: { row: any }) => {
        const item = row.original as FetchedRequestType;
        return <div className="capitalize">{item?.department || "-"}</div>;
      },
    },
    {
      header: () => "Status",
  ...
  const requestCards = useMemo(() => {
    ...
    requestsCountStatus?.total_count,
    requestsCountStatus?.pending_req,
    requestsCountStatus?.approve_req,
    requestsCountStatus?.decline_req,
  ]);
  return (
    <div className="flex flex-col gap-5 px-4 pt-3 md:pt-5 pb-5 md:pb-10 view-page-container overflow-y-scroll">
      ...
      <ViewRequestModal
        activeRequest={toggleModals.activeRequest!}
        isOpen={toggleModals.openViewRequestModal}
        onClose={() =>
          setToggleModals((prev) => ({
            ...prev,
            openViewRequestModal: false,
      ...
      <UpdateRequestStatusModal
        activeRequest={toggleModals.activeRequest!}
        isOpen={toggleModals.openRequestStatusModal}
        onClose={() =>
          setToggleModals((prev) => ({
            ...prev,
            openRequestStatusModal: false,
