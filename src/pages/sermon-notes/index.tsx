...
import {
  ...
  DeleteSermonModal,
  EditSermonModal,
} from "@/components/pages/sermon-notes";
import {
  getPaginationParams,
  setPaginationParams,
...
export const SermonNotesPage: React.FC = () => {
  ...
  const { data, isLoading } = useGetSermonNotes<FetchedSermonType[]>({
    ...filters,
    page: page.toString(),
    item_per_page: itemsPerPage.toString(),
  });
  const { data: adminsCount, isLoading: loadingSermonsCount } =
    useGetSermonNotes<FetchedAdminCountType>({
      component: "count",
      ...filters,
    });
  ...
  const toggleEditSermon = useCallback(
    (item: FetchedSermonType | null) => {
  ...
  const toggleDeleteSermon = useCallback(
    (item: FetchedSermonType | null) => {
      setToggleModals((prev) => ({
        ...prev,
        activeSermon: item,
                  ...
                  <div className="grid gap-1">
                    ...
                    <RenderIf condition={permission.includes("delete")}>
                      <MenuItem
                        ...
                        type="button"
                        className="flex items-center w-full rounded hover:bg-red-5 px-2 py-1.5 text-sm/6 text-text-secondary"
                        onClick={() => toggleDeleteSermon(item)}
                      >
                        Delete
                      </MenuItem>
                    </RenderIf>
  ...
  return (
    <div className="flex flex-col gap-5 px-4 pt-3 md:pt-5 pb-5 md:pb-10 view-page-container overflow-y-scroll">
        ...
        <div className="flex items-center gap-4 w-full sm:w-auto">
          ...
          <RenderIf condition={permission.includes("create")}>
            <TableAction
              ...
              theme="primary"
              onClick={() =>
                setToggleModals((prev) => ({ ...prev, openCreateSermon: true }))
              }
              block
            >
      ...
      <CreateSermonModal
        isOpen={toggleModals.openCreateSermon}
        onClose={() =>
          setToggleModals((prev) => ({ ...prev, openCreateSermon: false }))
        }
      />
      <DeleteSermonModal
        isOpen={toggleModals.openDeleteSermon}
        onClose={() => toggleDeleteSermon(null)}
        sermon={toggleModals.activeSermon as FetchedSermonType}
      />
      <EditSermonModal
        isOpen={toggleModals.openEditSermon}
        onClose={() => toggleEditSermon(null)}
        sermon={toggleModals.activeSermon as FetchedSermonType}
      />
    </div>
  );
